...
export function normalizeVoucher(item) {
  ...
  return {
    ...
    requiredTierId:
      item.requiredTierId != null ? Number(item.requiredTierId) : null,
    requiredTierName:
      item.requiredTierName != null ? String(item.requiredTierName) : null,
    validStartTime:
      item.validStartTime != null ? String(item.validStartTime) : null,
    validEndTime: item.validEndTime != null ? String(item.validEndTime) : null,
    vehicleTypeId:
      item.vehicleTypeId != null ? Number(item.vehicleTypeId) : null,
    discountPercent:
...
export function toApiExpiryDate(value) {
  ...
  const noZ = value.endsWith("Z") ? value.slice(0, -1) : value;
  const normalized = noZ.length === 16 ? `${noZ}:00` : noZ;
  return normalized;
}
...
export function buildVoucherPayload(voucher, overrides = {}) {
  ...
  const payload = {
    ...
    imageUrl: merged.imageUrl?.trim?.()
      ? String(merged.imageUrl).trim()
  ...
  if (isPercent && Number(merged.discountPercent ?? 0) > 0) {
    ...
    payload.maxDiscountAmount = Number(
      merged.maxDiscountAmount ?? merged.discountAmount ?? 0,
    );
  }
