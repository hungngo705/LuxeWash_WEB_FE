...
const dangerButtonClass = 'inline-flex h-10 items-center justify-center gap-1.5 rounded-xl border border-error/25 px-3 text-sm font-semibold text-error transition-colors hover:bg-error-container/20 disabled:cursor-not-allowed disabled:opacity-40'
...
export default function ManagerInventoryPage() {
  ...
  const materialOptions = useMemo(() => {
    const map = new Map()
    materials.forEach((m) => map.set(Number(m.materialId), { id: Number(m.materialId), name: m.name, unit: m.unit }))
    stocks.forEach((s) => map.set(Number(s.materialId), { id: Number(s.materialId), name: s.materialName, unit: s.unit }))
    batches.forEach((b) => {
      if (!map.has(Number(b.materialId))) map.set(Number(b.materialId), { id: Number(b.materialId), name: b.materialName, unit: '' })
    })
    return [...map.values()].sort((a, b) => a.name.localeCompare(b.name))
  }, [materials, stocks, batches])

  const selectedImportMaterial = useMemo(
    () => materialOptions.find((material) => Number(material.id) === Number(importForm.materialId)),
    [importForm.materialId, materialOptions],
  )

  const selectedAdjustMaterial = useMemo(
    () => materialOptions.find((material) => Number(material.id) === Number(adjustForm.materialId)),
    [adjustForm.materialId, materialOptions],
  )

  const importTotalCost = Number(importForm.quantity || 0) * Number(importForm.unitCost || 0)
  ...
  const loadActive = useCallback(async () => {
    ...
    try {
      ...
      if (activeTab === 'batches') {
        ...
        setTransactions(await fetchManagerInventoryTransactions({
          materialId: filters.materialId || undefined,
        ...
        setExtraRequests(await fetchManagerExtraUsageRequests(filters.extraStatus || undefined))
      } else if (activeTab === 'report') {
        setReport(await fetchManagerInventoryReport({
          from: filters.from || undefined,
          to: filters.to || undefined,
        }))
      }
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'Không tải được dữ liệu kho.')
    } finally {
      setLoading(false)
    }
  }, [activeTab, filters.extraStatus, filters.from, filters.includeDepleted, filters.materialId, filters.to, filters.txType, loadBase])
  ...
  const saveImport = async () => {
    ...
    try {
      ...
      if (importForm.manufactureDate && importForm.expiryDate && importForm.manufactureDate > importForm.expiryDate) {
  ...
  const saveAdjustment = async () => {
    ...
    try {
      await adjustManagerInventoryStock({
        ...
        materialBatchId: adjustForm.materialBatchId ? Number(adjustForm.materialBatchId) : null,
  ...
  const discardBatch = async (batch) => {
    ...
    const reason = window.prompt(`Lý do hủy batch ${batch.batchCode}?`, 'Hết hạn/không còn sử dụng')
  ...
  const reviewExtra = async (request, action) => {
    ...
    const managerNote = window.prompt(`${action === 'approve' ? 'Duyệt' : 'Từ chối'} request #${request.requestId}`, '')
    if (managerNote == null) return
    setReviewingRequestId(request.requestId)
    try {
      if (action === 'approve') {
        await approveManagerExtraUsageRequest(request.requestId, { managerNote })
      } else {
  ...
  return (
    <div className="w-full">
      ...
      {(activeTab === 'batches' || activeTab === 'transactions' || activeTab === 'report' || activeTab === 'extra') && (
          ...
          <div className="flex flex-wrap items-end gap-3 p-4">
            {(activeTab === 'batches' || activeTab === 'transactions') && (
              <label className="block min-w-56 flex-1 space-y-1 md:flex-none">
                ...
                <select className={fieldClass} value={filters.materialId} onChange={(e) => setFilters((f) => ({ ...f, materialId: e.target.value }))}>
            ...
            {activeTab === 'batches' && (
              <label className="flex h-11 items-center gap-2 rounded-xl border border-outline-variant bg-surface-container-lowest px-3 text-sm font-medium text-on-surface">
                <input type="checkbox" className={checkboxClass} checked={filters.includeDepleted} onChange={(e) => setFilters((f) => ({ ...f, includeDepleted: e.target.checked }))} />
            ...
            {activeTab === 'transactions' && (
              <>
                <label className="block min-w-48 space-y-1">
                  ...
                  <select className={fieldClass} value={filters.txType} onChange={(e) => setFilters((f) => ({ ...f, txType: e.target.value }))}>
                    <option value="">Tất cả loại</option>
                    {['BranchImport', 'Usage', 'ExtraUsage', 'Discard', 'Adjustment'].map((type) => <option key={type} value={type}>{type}</option>)}
                  </select>
                </label>
                <label className="block min-w-44 space-y-1">
                  <span className={labelTextClass}>Từ ngày</span>
                  <input type="date" className={fieldClass} value={filters.from} onChange={(e) => setFilters((f) => ({ ...f, from: e.target.value }))} />
                </label>
                <label className="block min-w-44 space-y-1">
                  <span className={labelTextClass}>Đến ngày</span>
                  <input type="date" className={fieldClass} value={filters.to} onChange={(e) => setFilters((f) => ({ ...f, to: e.target.value }))} />
                </label>
              </>
            )}
            {activeTab === 'report' && (
            ...
            {activeTab === 'extra' && (
              <label className="block min-w-48 space-y-1">
                ...
                <select className={fieldClass} value={filters.extraStatus} onChange={(e) => setFilters((f) => ({ ...f, extraStatus: e.target.value }))}>
      ...
      {loading ? (
        ...
        <>
            ...
            <div className="grid gap-4 lg:grid-cols-3">
                  ...
                  <div className="p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <p className="truncate font-sora text-lg font-semibold text-on-surface">{s.materialName}</p>
                        <p className="text-xs text-on-surface-variant">{s.warehouseName}</p>
                      </div>
                      {s.isLowStock ? <StatusBadge status="Low stock" /> : <StatusBadge status="OK" />}
                    </div>
                    <p className="mt-5 font-sora text-3xl font-semibold text-primary">
                      {num(s.currentQuantity)} <span className="text-sm font-medium text-on-surface-variant">{s.unit}</span>
                    </p>
                    <div className="mt-3 rounded-xl bg-surface-container-low px-3 py-2 text-xs text-on-surface-variant">
                      Tối thiểu: {num(s.minStockLevel)} · Cập nhật {formatDateTime(s.updatedAt)}
                    </div>
                  </div>
                </Panel>
              ))}
              {stocks.length === 0 && (
                <div className="lg:col-span-3">
                  <Panel>
                    <EmptyState title="Chưa có tồn kho" description="Nhập batch đầu tiên để bắt đầu theo dõi vật tư tại chi nhánh." />
                  </Panel>
                </div>
              )}
            </div>
          ...
          {activeTab === 'imports' && (
            <Panel>
              <div className="border-b border-outline-variant bg-surface-container-low px-4 py-3">
                <p className="font-sora text-base font-semibold text-on-surface">Nhập lô vật tư</p>
                <p className="text-xs text-on-surface-variant">Ghi nhận batch mới, giá vốn và hạn sử dụng để hệ thống theo dõi FIFO.</p>
              </div>
              <form className="grid gap-4 p-4 md:grid-cols-3" onSubmit={(e) => { e.preventDefault(); saveImport() }}>
                <label className="block space-y-1"><span className={labelTextClass}>Vật tư</span><select required className={fieldClass} value={importForm.materialId} onChange={(e) => setImportForm((f) => ({ ...f, materialId: e.target.value }))}><option value="">Chọn vật tư</option>{materialOptions.map((m) => <option key={m.id} value={m.id}>{m.name} {m.unit ? `(${m.unit})` : ''}</option>)}</select></label>
                <label className="block space-y-1"><span className={labelTextClass}>Mã batch</span><input required className={fieldClass} value={importForm.batchCode} onChange={(e) => setImportForm((f) => ({ ...f, batchCode: e.target.value }))} /></label>
                <label className="block space-y-1"><span className={labelTextClass}>Số lượng</span><input type="number" min="0" step="0.0001" required className={fieldClass} value={importForm.quantity} onChange={(e) => setImportForm((f) => ({ ...f, quantity: e.target.value }))} /></label>
                <label className="block space-y-1"><span className={labelTextClass}>Giá vốn / đơn vị</span><input type="number" min="0" step="0.0001" required className={fieldClass} value={importForm.unitCost} onChange={(e) => setImportForm((f) => ({ ...f, unitCost: e.target.value }))} /></label>
                <label className="block space-y-1"><span className={labelTextClass}>Ngày sản xuất</span><input type="date" className={fieldClass} value={importForm.manufactureDate} max={importForm.expiryDate || undefined} onChange={(e) => setImportForm((f) => ({ ...f, manufactureDate: e.target.value }))} /></label>
                <label className="block space-y-1"><span className={labelTextClass}>Hạn sử dụng</span><input type="date" className={fieldClass} value={importForm.expiryDate} min={todayDateOnly()} onChange={(e) => setImportForm((f) => ({ ...f, expiryDate: e.target.value }))} /></label>
                <label className="block space-y-1 md:col-span-2"><span className={labelTextClass}>Nhà cung cấp</span><input className={fieldClass} value={importForm.supplierName} onChange={(e) => setImportForm((f) => ({ ...f, supplierName: e.target.value }))} /></label>
                <div className="rounded-xl border border-outline-variant bg-surface-container-low px-3 py-2 text-sm">
                  ...
                  <p className="mt-1 font-sora text-lg font-semibold text-on-surface">{formatVnd(importTotalCost)}</p>
                  <p className="text-xs text-on-surface-variant">{selectedImportMaterial?.unit ? `Đơn vị: ${selectedImportMaterial.unit}` : 'Chọn vật tư để xem đơn vị'}</p>
                </div>
          ...
          {activeTab === 'adjustments' && (
            <Panel>
              <div className="border-b border-outline-variant bg-surface-container-low px-4 py-3">
                <p className="font-sora text-base font-semibold text-on-surface">Điều chỉnh tồn kho</p>
                <p className="text-xs text-on-surface-variant">Tăng hoặc giảm tồn theo batch cụ thể, hoặc để hệ thống trừ theo FIFO.</p>
              </div>
              <form className="grid gap-4 p-4 md:grid-cols-4" onSubmit={(e) => { e.preventDefault(); saveAdjustment() }}>
                <label className="block space-y-1"><span className={labelTextClass}>Vật tư</span><select required className={fieldClass} value={adjustForm.materialId} onChange={(e) => setAdjustForm((f) => ({ ...f, materialId: e.target.value, materialBatchId: '' }))}><option value="">Chọn vật tư</option>{materialOptions.map((m) => <option key={m.id} value={m.id}>{m.name} {m.unit ? `(${m.unit})` : ''}</option>)}</select></label>
                <label className="block space-y-1"><span className={labelTextClass}>Batch</span><select className={fieldClass} value={adjustForm.materialBatchId} onChange={(e) => setAdjustForm((f) => ({ ...f, materialBatchId: e.target.value }))}><option value="">Tự trừ FIFO</option>{batches.filter((b) => !adjustForm.materialId || Number(b.materialId) === Number(adjustForm.materialId)).filter((b) => Number(b.remainingQuantity) > 0).map((b) => <option key={b.materialBatchId} value={b.materialBatchId}>{b.batchCode} - còn {num(b.remainingQuantity)}</option>)}</select></label>
                <label className="block space-y-1"><span className={labelTextClass}>Tăng/giảm</span><input type="number" step="0.0001" required className={fieldClass} value={adjustForm.quantityChange} onChange={(e) => setAdjustForm((f) => ({ ...f, quantityChange: e.target.value }))} /></label>
                <label className="block space-y-1"><span className={labelTextClass}>Lý do</span><input required className={fieldClass} value={adjustForm.reason} onChange={(e) => setAdjustForm((f) => ({ ...f, reason: e.target.value }))} /></label>
                <div className="rounded-xl border border-outline-variant bg-surface-container-low px-3 py-2 text-sm md:col-span-4">
                  ...
                  <p className="mt-1 font-medium text-on-surface">
                    {selectedAdjustMaterial?.name || 'Chưa chọn vật tư'}: {Number(adjustForm.quantityChange || 0) > 0 ? '+' : ''}{num(adjustForm.quantityChange || 0)} {selectedAdjustMaterial?.unit || ''}
                  </p>
                  <p className="text-xs text-on-surface-variant">{adjustForm.materialBatchId ? 'Điều chỉnh batch đã chọn.' : 'Nếu giảm tồn mà không chọn batch, hệ thống sẽ trừ theo FIFO.'}</p>
                </div>
