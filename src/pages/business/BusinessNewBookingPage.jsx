...
import { fetchFleetVehicles, getAvailableSlots, createBusinessBooking } from '../../api/business.api'
import { fetchBranches } from '../../api/admin.branches.api'
import { fetchServices } from '../../api/admin.services.api'
import { formatVnd } from '../../utils/format'

const STEPS = ['Chọn xe', 'Chọn chi nhánh', 'Chọn ngày & slot', 'Chọn dịch vụ & xác nhận']
...
export default function BusinessNewBookingPage() {
  ...
  useEffect(() => {
    Promise.all([fetchFleetVehicles(), fetchBranches(), fetchServices()])
      .then(([v, b, s]) => {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    getAvailableSlots(selectedBranch, selectedDate)
  ...
  const totalPrice = selectedServices.reduce((sum, id) => {
    const svc = services.find((s) => s.serviceId === id || s.id === id)
  ...
  const handleSubmit = async () => {
    ...
    try {
      const dto = {
        fleetVehicleId: selectedVehicle.fleetVehicleId || selectedVehicle.id,
      ...
      <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant p-6">
        ...
        {step === 0 && (
          <div className="space-y-3">
            ...
            {vehicles.length === 0 ? (
              <p className="text-sm text-on-surface-variant">Chưa có xe nào được duyệt.</p>
            ) : (
              vehicles.map((vehicle) => (
                <button
                  key={vehicle.fleetVehicleId || vehicle.id}
        ...
        {step === 1 && (
          <div className="space-y-3">
            <h3 className="font-medium text-on-surface mb-3">Chọn chi nhánh</h3>
            {branches.map((branch) => (
              <button
                key={branch.branchId || branch.id}
                onClick={() => setSelectedBranch(branch.branchId || branch.id)}
                className={`w-full text-left p-4 rounded-xl border transition-colors ${
                  selectedBranch === (branch.branchId || branch.id)
                    ? 'border-primary bg-primary/5'
                    : 'border-outline-variant hover:border-primary/50'
                }`}
              >
            ...
            {selectedDate && (
              <div>
                ...
                {loading ? (
                  ...
                  <p className="text-sm text-on-surface-variant">Không có slot trống cho ngày này.</p>
                ) : (
                    ...
                    {slots.map((slot) => (
                      <button
                        ...
                        className={`p-2 rounded-lg border text-xs font-medium transition-colors ${
                          selectedSlot?.slotId === slot.slotId || selectedSlot?.id === slot.id
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-2">
                {services.map((service) => (
                  <button
                    ...
                    onClick={() => toggleService(service.serviceId || service.id)}
                    className={`w-full text-left p-4 rounded-xl border transition-colors flex items-center justify-between ${
                      selectedServices.includes(service.serviceId || service.id)
                        ? 'border-primary bg-primary/5'
                        : 'border-outline-variant hover:border-primary/50'
                    }`}
                  >
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-sm font-semibold text-primary">{formatVnd(service.price)}</span>
                      {selectedServices.includes(service.serviceId || service.id) && (
            ...
            <div className="bg-surface-container rounded-xl p-4 space-y-2">
              ...
              <div className="text-sm text-on-surface-variant space-y-1">
                <p>Xe: <span className="text-on-surface">{selectedVehicle?.licensePlate}</span></p>
                <p>Chi nhánh: <span className="text-on-surface">{branches.find(b => (b.branchId || b.id) === selectedBranch)?.name}</span></p>
                <p>Ngày: <span className="text-on-surface">{selectedDate}</span></p>
                <p>Slot: <span className="text-on-surface">{selectedSlot?.startTime} — {selectedSlot?.endTime}</span></p>
                <p>Dịch vụ: <span className="text-on-surface">{selectedServices.length} dịch vụ</span></p>
              </div>
